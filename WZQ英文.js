...
function 保存名称() {
  ...
  if (!用户名) {
    弹出信息("Please enter your name", "red"); // 提示用户输入
    return;
  }
...
function 主机端初始化() {
  ...
  主机端
    ...
    .then((a) => {
      弹出信息("Host connection set up successfully");
    });
}
//客户端初始化，生成RTC连接
function 客户端初始化(主机端连接信息) {
  ...
  客户端.ondatachannel = function (e) {
    ...
    客户端.客户端频道.onopen = function (e) {
      弹出信息("Client connection established");
  ...
  客户端
    ...
    .then((a) => 弹出信息("Client connected to host successfully"));
  客户端
    .createAnswer()
    .then((a) => 客户端.setLocalDescription(a))
    .then((a) => 弹出信息("Client reply to host set successfully"));
}
...
function 对方连接信息确认() {
  ...
  弹出信息("Please select host or client first");
  return false;
}
...
function 显示游戏数据() {
  ...
  if (全局游戏数据.结束 != "") {
    ...
    tstr =
      "Current turn: " + (全局游戏数据.下棋方 === "主机端" ? "Host" : "Client");
    twidth = ctx.measureText(tstr).width;
    ty = 总高度 - 2 * 基础宽高;
    ctx.fillText(tstr, tx - twidth / 2, ty - theight);
    ...
    if (
      ...
      tstr = "It's your turn (";
      if (全局游戏数据.下棋方 === "主机端") {
        tstr = "It's your turn (Black)";
      } else {
        tstr = "It's your turn (White)";
      }
      twidth = ctx.measureText(tstr).width;
      ty = 总高度 - 10 * 基础宽高;
      ctx.fillText(tstr, tx - twidth / 2, ty - theight);
      ...
      tstr = "Please wait (";
      if (全局游戏数据.下棋方 === "主机端") {
        tstr = "Please wait (White)";
      } else {
        tstr = "Please wait (Black)";
      }
      twidth = ctx.measureText(tstr).width;
      ty = 总高度 - 10 * 基础宽高;
      ctx.fillText(tstr, tx - twidth / 2, ty - theight);
